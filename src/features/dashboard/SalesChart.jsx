...
const SalesChart = function ({ bookings = [], numDays }) {
  ...
  const dayBookings = (date) =>
    bookings.filter((booking) => isSameDay(date, new Date(booking.created_at)));

  const data = allDates.map((date) => {
    return {
      label: format(date, "MMM dd"),
      totalSales: dayBookings(date).reduce(
        (acc, cur) => acc + cur.totalPrice,
        0,
      ),
      extrasSales: dayBookings(date).reduce(
